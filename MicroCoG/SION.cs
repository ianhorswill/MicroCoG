...
namespace SomaSim.SION
{
    ...
    public class SIONException : Exception
    {
        ...
        private void SetContext (SION.ParseContext c) {
            ...
            this._context = "Parse Context: " + ((read > 0) ? new string(buffer) : "(???)");
        }

        private void SetContext (SION.PrintContext c) {
            string tail = c.sb.ToString();
            tail = tail.Substring(System.Math.Max(0, tail.Length - 20));
            this._context = "Print Context: " + tail;
        }
    }
    ...
    public class SION
    {
        ...
        /** OLDIES
        private const string NUMBER_START = "1234567890+-";
        private const string NUMBER_BODY = "1234567890.";

        private const string SIMPLE_STRING_START = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_";
        private const string SIMPLE_STRING_BODY = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-1234567890.";

        private static bool IsStartOfNumber (char ch) { return NUMBER_START.IndexOf(ch) >= 0; }
        private static bool IsBodyOfNumber (char ch) { return NUMBER_BODY.IndexOf(ch) >= 0; }

        private static bool IsStartOfSimpleString (char ch) { return SIMPLE_STRING_START.IndexOf(ch) >= 0; }
        private static bool IsBodyOfSimpleString (char ch) { return SIMPLE_STRING_BODY.IndexOf(ch) >= 0; }
        */
        ...
        private static object ParseNumber(ParseContext c)
        {
            ...
            if (int.TryParse(token, out var result))
                return result;
            if (float.TryParse(token, NumberStyles.Number, CultureInfoInvariantCulture, out float fResult))
                return fResult;
            throw new SIONException(null, c, "Unknown number format in " + token);
        }

        private static object ParseSimpleString (ParseContext c) {
            return ConsumeWhile(c, IsBodyOfSimpleString, true);
        }
        ...
        private static bool PrintPrimitive (object data, PrintContext c) {
            ...
            if (data is bool) {
                c.sb.Append(TOKEN_BOOL_OR_NULL_START);
                c.sb.Append((bool)data ? "true" : "false");
                return true;
            }
        ...
        private static string FormatFloatingPoint (object data) {
            ...
            if (candidate.Length >= 2 && candidate[candidate.Length - 1] == '.') { // ugly hackery
